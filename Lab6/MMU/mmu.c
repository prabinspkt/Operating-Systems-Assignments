...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
	...
	// Check if there is a node with enough block size. 
	...
	if (!available_block_node){
		...
		if (!prev){
			freelist->head = freelist->head->next; // Remove the head node.  
		}else{
	...
	// Update the available_block with the new params for the process. 
	...
	// Handle the fragmented memory. 
	if (available_block_node->blk->end != blk_end){
		block_t *fragment = (block_t*)malloc(sizeof(block_t));
		fragment->pid = 0;
		fragment->start = available_block_node->blk->end + 1;
		fragment->end = blk_end;
	
		// Add to the free list based on policy. 
		if (policy == 1){
