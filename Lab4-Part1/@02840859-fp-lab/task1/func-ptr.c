...
{
  const Process *process_one = this; 
  const Process *process_two = that;
  if (process_one->priority > process_two->priority) return -1;
  if (process_one->priority == process_two->priority) {
    if (process_one->arrival_time < process_two->arrival_time) return -1;
    if (process_one->arrival_time > process_two->arrival_time) return 1;
    if (process_one->arrival_time == process_two->arrival_time) return 0;
  }
  if (process_one->priority < process_two->priority) return 1;
  return 0;
}
